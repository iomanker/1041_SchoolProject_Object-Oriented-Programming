...
void HandleEvent(int index, WSAEVENT *Handles, Socklist *socklist)
{
	...
	{ // WSARecv() was queued
		...
		string sendstr = WaitAndReply(socklist[index].DataBuf.buf);
		const char* sendchar = sendstr.c_str();
		for (int j = 0; j < socklist[index].DataBuf.len - 1; j++)
		{
			socklist[index].DataBuf.buf[j] = sendchar[j];
		}
		socklist[index].DataBuf.buf[socklist[index].DataBuf.len - 1] = 0;
